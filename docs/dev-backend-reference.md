...
| `cef_cdp.py` | Chrome DevTools Protocol client for Steam's CEF. Reads open store/library pages for AppID auto-detect, reads the LIVE cookie store (how the LuaTools login is captured the moment it lands, before CEF flushes it to disk), and deletes cookies on logout. Also `HiddenView`: an off-screen BrowserView created through SharedJSContext, found on the debug port by a placeholder URL, navigated and driven with `Runtime.evaluate` (`fetch_in_page` runs fetch() inside the page, as the browser); `ExistingView` does the same on a tab the user already has open. |
...
| `steamdb_reader.py` | Reads SteamDB for `versions.py` (Cloudflare answers the backend with a JS challenge, so the page has to be fetched by Steam's own browser). Per URL: cache (`~/.cache/lumadeck/steamdb/`, feed 1 h / depot 6 h / build page 24 h) → one direct GET (skipped for an hour after a challenge) → fetch() inside a page on steamdb.info via `cef_cdp` (the user's SteamDB tab if open, else a hidden view navigated to the app's page and given 20 s to pass the challenge). A challenge the hidden view cannot pass is reported as `needs_user` (open SteamDB visibly once); challenge pages, HTTP errors and empty bodies are never cached. `probe(appid)` (Settings → Dev) reads feed + installed depots' histories + the oldest build's page, runs the translator on it and logs every fetch's transport / ms / outcome; result also in `probe_<appid>.json`. |
